# -*- coding: utf-8 -*-
"""Intact / subunit mass analysis adapter (P7)."""
...
from app.analysis.intact_mass.pipeline import (
    build_analysis_result,
    build_input_evidence,
    run_synthetic_demo,
    run_uploaded_spectra,
    utc_now,
)
...
class IntactMassAdapter(AnalysisAdapter):
    profile = AnalysisProfileId.INTACT_MASS.value
    ...
    def _run_sync(self, context: AdapterContext) -> AnalysisResult:
        ...
        if candidate_files and reference_files and sequence_files:
            pipeline = run_uploaded_spectra(
                item_id=context.job.item_id,
                sequence_path=sequence_files[0],
                reference_spectrum_path=reference_files[0],
                candidate_spectrum_path=candidate_files[0],
        ...
        return build_analysis_result(
            ...
            trace_id=f"intact-mass-{uuid4().hex[:12]}",
