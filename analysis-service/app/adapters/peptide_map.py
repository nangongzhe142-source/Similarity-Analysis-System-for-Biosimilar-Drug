# -*- coding: utf-8 -*-
"""Peptide-map TIC overlay adapter (P12)."""
...
from app.analysis.peptide_map.pipeline import (
    build_analysis_result,
    build_input_evidence,
    run_synthetic_demo,
    run_uploaded_chromatograms,
    utc_now,
)
...
class PeptideMapAdapter(AnalysisAdapter):
    profile = AnalysisProfileId.PEPTIDE_MAP.value
    ...
    def _run_sync(self, context: AdapterContext) -> AnalysisResult:
        ...
        if candidate_files and reference_files:
            pipeline = run_uploaded_chromatograms(
            ...
            pipeline = run_synthetic_demo(work_dir=artifacts_dir)
            synthetic_demo = True
        ...
        return build_analysis_result(
            ...
            trace_id=f"peptide-map-{uuid4().hex[:12]}",
