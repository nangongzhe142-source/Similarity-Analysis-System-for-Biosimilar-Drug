# -*- coding: utf-8 -*-
"""MS/MS sequence confirmation adapter (P9)."""
...
from app.analysis.msms.pipeline import (
...
class MsmsSequenceAdapter(AnalysisAdapter):
    profile = AnalysisProfileId.MSMS_SEQUENCE.value
    ...
    def _run_sync(self, context: AdapterContext) -> AnalysisResult:
        ...
        if sequence_files and candidate_files:
            pipeline = run_comet_uploaded(
                item_id=context.job.item_id,
                sequence_path=sequence_files[0],
                reference_spectra_path=reference_files[0] if reference_files else None,
                candidate_spectra_path=candidate_files[0],
        ...
        return build_analysis_result(
            ...
            trace_id=f"msms-sequence-{uuid4().hex[:12]}",
